function [img, tumor_x, tumor_y] = compute_beamform(params)
    ...
    % -------------------------------
    % PHYSICAL CONCEPT: Before the signal enters the breast tissue, it travels through
    % the antenna feed and any physical gap (e.g., matching liquid or air).
    % We must account for the propagation speed in these specific media (v_feed)
    % to correctly calculate the total time-of-flight (extra_delay).
    ...
    % Clutter Removal
    % -------------------------------
    % PHYSICAL CONCEPT: The skin/air interface produces a massive reflection that
    % completely masks the weak backscatter from the tumor. Clutter removal isolates
    % the tumor response.
    if time_gating_cr == 1
        % PHYSICAL CONCEPT: Time gating mutes early-arriving signals (which correspond
        % to skin reflections) and late-arriving signals (multi-path reflections),
        % keeping only the time window where the tumor reflection is expected.
        fprintf("Time gating\n");
        signals = mimt.clutter_removal.time_gating(signals, 50);
    end

    if av_sub_cr == 1
        % PHYSICAL CONCEPT: Assuming the breast is mostly homogeneous fat and the antenna array
        % is symmetric, the skin reflection is similar across channels. Subtracting the average
        % removes this common-mode skin signal.
    ...
    if svd_cr == 1
        % PHYSICAL CONCEPT: SVD separates the data into orthogonal spatial modes. The first few modes
        % with the largest singular values represent the dominant, highly correlated skin reflection.
        % By discarding these, we are left with the weaker, localized tumor signal.
    ...
    % -------------------------------
    % PERFORMANCE ENHANCEMENT: Moving back and forth between time and frequency domains
    % using custom fd2td/td2fd can be slow. If native MATLAB `fft` / `ifft` are used,
    % ensure the time/frequency vectors are padded to powers of 2 (e.g., 512, 1024)
    % to maximize FFT efficiency.
    if time_domain_processing == 1
