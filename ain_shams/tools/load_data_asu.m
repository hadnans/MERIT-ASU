function [scan2, scan1, frequencies, sensors_locations, channel_names] = load_data_asu(data, conf_pol, channels_mode, data_dir)
    % Input 'data_dir' is optional. Used when data == 3.

% % load data
% data : 0 => simulation , 1 => measured, 2 => all, 3 => custom directory
% conf_pol : 11, 12, 21, 22
% channels_mode : 1 | 2 | 3 % select mono/multi static system

if data == 3
    % Load dynamically from user-specified directory
    % Expected files: scan1.csv, scan2.csv, channel_names.csv, frequencies.csv, antenna_locations.csv
    scan1 = readmatrix(fullfile(data_dir, 'scan1.csv'));
    scan2 = readmatrix(fullfile(data_dir, 'scan2.csv'));
    channel_names = readmatrix(fullfile(data_dir, 'channel_names.csv'));
    frequencies = readmatrix(fullfile(data_dir, 'frequencies.csv')) * 1e9;
    sensors_locations = readmatrix(fullfile(data_dir, 'antenna_locations.csv')) * 1e-3;
